...
async function renderBrowse() {
  ...
  <form id="browseFilters" class="filters"><input name="search" value="${escapeHtml(current.search)}" placeholder="Search an item or brand" />${state.user
    ...
    : `<select disabled aria-label="Sign in to filter by report type"><option>Lost & found</option></select><select disabled aria-label="Sign in to filter by category"><option>All categories</option></select><select disabled aria-label="Sign in to filter by location"><option>All locations</option></select>`}<button class="button button-small" type="submit">Search</button></form>
