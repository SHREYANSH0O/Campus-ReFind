...
async function renderBrowse() {
  ...
  <form id="browseFilters" class="filters"><input name="search" value="${escapeHtml(current.search)}" placeholder="Search an item or brand" />${state.user
    ? `<select name="type">${selectOptions(["lost", "found"], current.type, "Lost & found")}</select><select name="category">${selectOptions(categories, current.category, "All categories")}</select><select name="location">${selectOptions(locations, current.location, "All locations")}</select>`
    : `<button class="locked-select" type="button" data-action="sign-in" aria-label="Sign in to filter by report type"><span>Lost & found</span><b>⌄</b></button><button class="locked-select" type="button" data-action="sign-in" aria-label="Sign in to filter by category"><span>All categories</span><b>⌄</b></button><button class="locked-select" type="button" data-action="sign-in" aria-label="Sign in to filter by location"><span>All locations</span><b>⌄</b></button>`}<button class="button button-small" type="submit">Search</button></form>
...
async function showItem(id) {
  try {
    ...
    const publicDetail = item.details_protected
      ? `<div class="detail-content"><span class="type-badge ${item.report_type}">${typeLabel(item.report_type)} report</span><h2 style="margin-top:10px">${escapeHtml(item.title)}</h2><div class="item-meta"><span>⌖ ${escapeHtml(item.location)}</span><span>◷ ${dateLabel(item.reported_date)}</span></div><div class="protected-panel"><p class="overline">Protected details</p><h3>Full report info is hidden for guests.</h3><p>${escapeHtml(item.protected_details_message || "Sign in to view the full report details and start a verified claim.")}</p>${state.user ? "" : '<button class="button button-full" data-action="sign-in">Sign in to continue <span>→</span></button>'}</div></div>`
      : `<div class="detail-content"><span class="type-badge ${item.report_type}">${typeLabel(item.report_type)} report</span><h2 style="margin-top:10px">${escapeHtml(item.title)}</h2><div class="item-meta"><span>⌖ ${escapeHtml(item.location)}</span><span>◷ ${dateLabel(item.reported_date)}</span><span>by ${escapeHtml(item.reporter_name)}</span></div><p>${escapeHtml(item.description)}</p><div class="detail-meta-grid"><div><span>Category</span><b>${escapeHtml(item.category)}</b></div><div><span>Color</span><b>${escapeHtml(item.color || "Not provided")}</b></div><div><span>Brand / model</span><b>${escapeHtml(item.brand || "Not provided")}</b></div><div><span>Report status</span><b>${escapeHtml(item.status)}</b></div></div>${item.can_claim ? `<button class="button" data-action="open-claim" data-id="${item.id}" data-title="${escapeHtml(item.title)}">Claim this item <span>→</span></button>` : item.is_owner ? '<p style="margin:0;color:var(--blue);font-size:11px;font-weight:800">This is your report.</p>' : '<p style="margin:0;color:var(--muted);font-size:11px">Sign in to begin a verified ownership claim.</p>'}</div>`;
    itemDialog.innerHTML = `<button class="close-dialog" data-close="itemDialog" aria-label="Close">×</button><div class="detail-grid">${itemImage(item, "detail-image")}${publicDetail}</div>${item.is_owner ? `<section class="matches-inline"><div class="section-heading"><div><p class="overline">Automatic matching</p><h2>Possible opposite reports</h2></div></div><div class="match-list">${matches.length ? matches.map((match) => `<div class="match-row"><div class="score-ring">${match.score}%</div><div><h3>${escapeHtml(match.title)}</h3><p>${escapeHtml(match.reasons.join(" · "))}</p></div><button class="small-link" data-action="view-item" data-id="${match.id}">Open</button></div>`).join("") : emptyState("No potential matches", "We will keep checking new reports for a match.")}</div></section>` : ""}`;
